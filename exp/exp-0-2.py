...
task_id = int(sys.argv[1]) # from 0 to 24
num_tasks = int(sys.argv[2]) # should be 5 * 5 = 25
assert num_tasks == 25
...
optimizer = torch.optim.Adam(model.parameters(), lr=1e-5) # reset optimizer
step = 0
...
torch.save({
    ...
    'eval_losses': eval_losses}, f'../../../orcd/pool/MultiHeads/outputs/exp-0-2-{task_id}.pt')